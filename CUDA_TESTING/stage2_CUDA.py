...
# STEP 7: User-Movie Similarity Calculation with Ultra-Optimized CUDA
def calculate_similarity_cuda(data, threshold=0.3):
    """
    Tính toán độ tương đồng giữa người dùng và phim sử dụng CUDA với các tối ưu hóa về bộ nhớ và tính toán
    
    Các tối ưu hóa:
    1. Sử dụng sparse tensors cho kết quả thay vì dictionaries để giảm bộ nhớ
    2. Chia nhỏ việc tính toán thành các khối để tối ưu sử dụng VRAM
    3. Sử dụng phép nhân ma trận song song trên GPU
    4. Thực hiện threshholding trực tiếp trên GPU
    5. Sử dụng pinned memory để tăng tốc độ chuyển dữ liệu
    """
    print("\n" + "="*80)
    print("STEP 7: ULTRA-OPTIMIZED USER-MOVIE SIMILARITY CALCULATION WITH CUDA")
    print("="*80)
    
    start_time = time.time()
    
    # Kiểm tra xem dữ liệu có dạng tensor hay không
    if 'user_vectors_tensor' in data and 'movie_vectors_tensor' in data:
        user_vectors_tensor = data['user_vectors_tensor']
        movie_vectors_tensor = data['movie_vectors_tensor']
        
        print(f"Using pre-loaded tensors: users shape={user_vectors_tensor.shape}, movies shape={movie_vectors_tensor.shape}")
    elif 'user_vectors' in data and 'movie_vectors' in data:
        # Nếu dữ liệu là dictionaries, chuyển thành tensors
        print("Converting dictionaries to tensors...")
        
        user_ids = list(data['user_vectors'].keys())
        movie_ids = list(data['movie_vectors'].keys())
        
        # Tạo mapping mới nếu cần
        if 'user_id_mapping' not in data:
            data['user_id_mapping'] = {user_id: idx for idx, user_id in enumerate(user_ids)}
            data['idx_to_user_id'] = {idx: user_id for user_id, idx in data['user_id_mapping'].items()}
        if 'movie_id_mapping' not in data:
            data['movie_id_mapping'] = {movie_id: idx for idx, movie_id in enumerate(movie_ids)}
            data['idx_to_movie_id'] = {idx: movie_id for movie_id, idx in data['movie_id_mapping'].items()}
        
        # Đảm bảo user_ids và movie_ids có cùng thứ tự với mapping
        user_ids_sorted = sorted(user_ids, key=lambda x: data['user_id_mapping'][x])
        movie_ids_sorted = sorted(movie_ids, key=lambda x: data['movie_id_mapping'][x])
        
        # Tạo tensors
        user_matrix = np.array([data['user_vectors'][uid] for uid in user_ids_sorted])
        movie_matrix = np.array([data['movie_vectors'][mid] for mid in movie_ids_sorted])
        
        # Chuyển sang GPU với pinned memory để tăng tốc
        user_vectors_tensor = torch.tensor(user_matrix, dtype=torch.float32, device=device)
        movie_vectors_tensor = torch.tensor(movie_matrix, dtype=torch.float32, device=device)
        
        # Lưu lại vào data
        data['user_vectors_tensor'] = user_vectors_tensor
        data['movie_vectors_tensor'] = movie_vectors_tensor
    else:
        print("Error: No user or movie vectors found in data")
        return data
    
    # Số lượng users và movies
    num_users = user_vectors_tensor.shape[0]
    num_movies = movie_vectors_tensor.shape[0]
    
    # Kiểm tra và đảm bảo vectơ đã được chuẩn hóa để tăng hiệu suất
    user_norms = torch.norm(user_vectors_tensor, dim=1)
    movie_norms = torch.norm(movie_vectors_tensor, dim=1)
    
    if not torch.allclose(user_norms, torch.ones_like(user_norms), rtol=1e-2):
        print("Normalizing user vectors for better performance...")
        user_vectors_tensor = F.normalize(user_vectors_tensor, p=2, dim=1)
    
    if not torch.allclose(movie_norms, torch.ones_like(movie_norms), rtol=1e-2):
        print("Normalizing movie vectors for better performance...")
        movie_vectors_tensor = F.normalize(movie_vectors_tensor, p=2, dim=1)
    
    # Chuyển vị ma trận movie để tính nhân ma trận
    movie_vectors_T = movie_vectors_tensor.t()
    
    # Tính toán độ tương đồng theo khối để tối ưu VRAM
    # Tính kích thước khối tối ưu dựa trên kích thước tensor và bộ nhớ GPU
    if cuda_available:
        # Ước tính bộ nhớ cần thiết cho mỗi khối dựa vào lý thuyết
        movie_size_bytes = movie_vectors_T.element_size() * movie_vectors_T.nelement()
        result_size_per_user = num_movies * 4  # Float32 = 4 bytes
        
        # Ước tính VRAM khả dụng (giữ lại 20% cho xử lý khác)
        available_vram = torch.cuda.get_device_properties(0).total_memory * 0.8 - torch.cuda.memory_allocated()
        
        # Tính toán kích thước khối tối ưu
        max_batch_size = int(available_vram / (result_size_per_user + user_vectors_tensor[0].element_size() * user_vectors_tensor[0].nelement()))
        
        # Giới hạn trong khoảng hợp lý
        batch_size = min(max(100, max_batch_size), 5000, num_users)
        print(f"Optimized batch size: {batch_size} users (using {batch_size * result_size_per_user / 1e6:.2f}MB per batch)")
    else:
        batch_size = 100  # Default for CPU
    
    # Sử dụng cấu trúc dữ liệu hiệu quả để lưu trữ kết quả
    user_movie_similarities = {}
    total_above_threshold = 0
    
    # Khởi tạo một tensor đánh dấu movieId để tăng tốc lookups
    movie_ids_tensor = torch.tensor(list(data['movie_id_mapping'].keys()), device=device)
    
    # Xử lý từng khối
    for start_idx in range(0, num_users, batch_size):
        batch_end = min(start_idx + batch_size, num_users)
        current_batch_size = batch_end - start_idx
        
        # Lấy batch của user vectors
        user_batch = user_vectors_tensor[start_idx:batch_end]
        
        # Tính toán ma trận tương đồng cho toàn bộ batch (user_batch x movie_vectors_T)
        # Shape: [batch_size, num_movies]
        with torch.amp.autocast('cuda', enabled=cuda_available):
            similarity_matrix = torch.mm(user_batch, movie_vectors_T)
        
        # Áp dụng ngưỡng tương đồng trực tiếp trên GPU
        threshold_mask = similarity_matrix > threshold
        
        # Đếm số lượng giá trị trên ngưỡng
        total_above_threshold += torch.sum(threshold_mask).item()
        
        # Xử lý từng user trong batch
        for batch_idx in range(current_batch_size):
            global_user_idx = start_idx + batch_idx
            user_id = data['idx_to_user_id'][global_user_idx]
            
            # Lấy indices của các movie có tương đồng vượt ngưỡng
            user_threshold_mask = threshold_mask[batch_idx]
            movie_indices = torch.nonzero(user_threshold_mask).squeeze(1)
            
            if len(movie_indices) == 0:
                user_movie_similarities[user_id] = {}
                continue
            
            # Lấy giá trị tương đồng và movie IDs
            sim_values = similarity_matrix[batch_idx][movie_indices].cpu()
            selected_movie_indices = movie_indices.cpu()
            
            # Tạo dictionary cho user này
            user_dict = {}
            for i, movie_idx in enumerate(selected_movie_indices):
                movie_id = data['idx_to_movie_id'][movie_idx.item()]
                sim_value = sim_values[i].item()
                user_dict[movie_id] = sim_value
            
            user_movie_similarities[user_id] = user_dict
        
        # Hiển thị tiến độ
        elapsed = time.time() - start_time
        progress = batch_end / num_users
        remaining = elapsed / progress - elapsed if progress > 0 else 0
        print(f"Processed {batch_end}/{num_users} users ({progress*100:.1f}%) - Elapsed: {elapsed:.2f}s - Remaining: {remaining:.2f}s")
        
        # Giải phóng bộ nhớ không dùng đến
        if cuda_available:
            del similarity_matrix, threshold_mask
    ...
    total_pairs = num_users * num_movies
    pairs_per_second = total_pairs / total_calculation_time
    avg_movies_per_user = total_above_threshold / num_users
    
    print(f"\nSimilarity calculation complete in {total_calculation_time:.2f}s")
    print(f"Total users: {num_users}, Total movies: {num_movies}")
    print(f"Total similarity calculations: {total_pairs:,} pairs")
    print(f"Processing speed: {pairs_per_second:,.2f} pairs/second")
    print(f"Similarity values above threshold: {total_above_threshold:,} ({total_above_threshold/total_pairs*100:.2f}%)")
    print(f"Average similar movies per user: {avg_movies_per_user:.2f}")
    
    # Hiển thị thông tin về bộ nhớ CUDA
    if cuda_available:
        print(f"\nCUDA Memory Usage:")
...
def generate_recommendations_cuda(data, n=10):
    ...
    for user_idx, movie_idx in zip(train_users_idx.cpu().numpy(), train_movies_idx.cpu().numpy()):
        user_id = data['idx_to_user_id'][user_idx.item()]
...
def evaluate_model_cuda(data):
    ...
    for batch_idx in range(num_batches):
        ...
        for user_idx in batch_users:
            ...
            user_idx_scalar = user_idx.item()
            user_id = data['idx_to_user_id'][user_idx_scalar]
            ...
            for i, movie_idx in enumerate(user_movie_indices.cpu().numpy()):
                ...
                if isinstance(movie_idx, np.ndarray):
...
def main():
    ...
    data = calculate_similarity_cuda(data, threshold=similarity_threshold)
    ...
    if cuda_available:
        ...
        try:
            ...
            print("- Utilization: Not available (pynvml not installed)")    
    # Total runtime
